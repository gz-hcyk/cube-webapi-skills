/**
 * Cube WebApi 前端 · 设计令牌（TypeScript 源 · demo 落地版）
 * -------------------------------------------------------------
 * 与 `src/styles/tokens.css` 单一事实来源保持一致，供图表配色、
 * 逻辑运算、动态样式等非 CSS 场景使用（如 ECharts 设备在线饼图）。
 *
 * 本 demo 采用「政务蓝」默认主题（--td-brand-color: #0f4c9e），
 * 故下列 brand / gradient 取值以 tokens.css 实际值为准，二者不得冲突。
 * 修改令牌时请同步维护 tokens.css。
 */
...
export const cyan = {
  ...
  active: '#0077ad',
  light: '#d9f1fb',
  /** 8 级浅色阶梯（对应 --cube-accent-cyan-1..8） */
...
/** 阴影（与 tokens.css --td-shadow-1/2/3 对应） */
export const shadow = {
...
/** 间距阶梯（px，对应 --cube-spacing-1..8） */
export const spacing = [4, 8, 12, 16, 24, 32, 48, 64] as const;

/** 字号阶梯（px，对应 --cube-font-size-xs..4xl） */
export const fontSize = [12, 14, 16, 18, 20, 24, 28, 36] as const;

/** 品牌渐变（CSS 字符串，对应 --cube-brand-gradient / --cube-brand-gradient-iot） */
