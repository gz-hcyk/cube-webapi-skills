...
- 代理目标错配 → 登录/菜单 500
- 实体接口必须写全 `/api` 前缀 → 漏写则全体 404（高频坑，2026-09 复测反转）
...
- **实体接口必须写全 `/api` 前缀 → 漏写则全体 404（高频坑，2026-09 复测反转）**：`src/api/http.ts` 的 `baseURL` 是 **`'/'`**（**不是** `'/api'`；旧文档写的 `baseURL:'/api'` 属技能早期 `api.ts`，该文件已删除，见 SKILL 铁律 H1）。故所有实体调用路径**必须自带 `/api` 前缀**：`useEntityResource` 的 `base` = `` `/api/${area}/${controller}` ``；`DetailDrawer`/`FormDialog` 复用该 base；`useLookups` 拉外键字典写 `/api/${a}/${ctrl}`，框架内置表（Dept/User/Role）回退 `/api/Admin/{控制器}`。**漏写前缀 → `/{area}/{ctrl}` 实测 404**，典型表现：**外键列只剩原始 ID、表单下拉为空**。`assets/core/api/useEntityResource.ts`、`assets/core/api/useLookups.ts`、`assets/core/components/cube/DetailDrawer.vue`、`assets/core/components/cube/FormDialog.vue` 已按此修正。**落地后务必全量 `grep` `src` 中 `getApi(`/`postApi(`/`putApi(`/`deleteApi(` 的调用，确认无残留 `/api/` 硬编码**（登录/菜单走 `rawHttp` 带 `/api` 是正确用法，勿误删）。
...
- **`rawHttp` 端点（`/Auth/*`、`/Mfa/*`）不带 `/api` 前缀，dev 代理须显式转发，否则登录 404（高频坑）**：`http.ts` 同时导出**两个独立 Axios 实例**：`rawHttp`（非实体端点：`/Auth/LoginConfig`/`Challenge`/`Login`/`Refresh`、`/Mfa/Verify`、菜单树 `/api/Admin/Index/GetMenuTree`）与实体 `http`（路径自带 `/api`），二者 `baseURL` 均为 `'/'`。若 `vite.config.ts` 只配 `/api` 转发，`rawHttp` 请求被 Vite 当 SPA 路由 → 返回 `index.html` 或 404，表现为「登录页拉不到 `LoginConfig`、登录直接失败」。正确做法：代理同时含 `/api`、`/Auth`、`/Mfa`、`/cube`、`/Content`（目标统一指后端/Mock）；见 §4.3 与 §4.15。
...
- **菜单 submenu value 必须稳定唯一，否则"点一个全展开"**：`t-submenu` 的 `:value` 切勿用会冲突的 `node.text||node.title`——当真实后端菜单字段名不是 `text/title`（NewLife 常用 `Name`/`Url`/`Childs`/`Icon`，且常 PascalCase）时，这些取值全部为空/`undefined` → 多个 submenu 同 value → 点击任意一个，所有同值菜单同步展开（表现为"点一个全展开"）。必须按 `优先 url → 每行 id → 层级路径(m-i/m-i-j)` 生成唯一 value；并设 `t-menu accordion` 手风琴（一次仅展开一个）+ 受控 `expanded` 与 `@expand` 同步。字段读取需大小写兜底（`text/Text/name/Name`、`children/Children/Childs`、`url/Url`…）。`assets/core/components/cube/MenuSidebar.vue` 已落地该健壮实现（含图标 + 设计系统激活态）。
...
- **📦 前端工程实践聚合（Vite / npm / 构建 / 脚手架 / 请求层，实测于 NewLife.Cube WebApi + Vue3 + TDesign 个人博客项目）**：以下前端专属落地细节统一归口本 skill（与 `cube-webapi-backend` 第十四节后端契约互补，避免前后端知识错置）：
  ...
  - **请求层三件套完整代码不再内嵌**：`http.ts` / `camel.ts` / `auth.ts`（token 存取、camelize 递归归一、axios 实例 + Bearer 注入 + 401 处理）真身在 `references/demo/src/api/`（demo 工程）与 `assets/`（拷贝模板源），直接拷贝该处代码，避免双份漂移。分页/门户两种 `data` 形状差异仍在业务层分支处理；后端契约（`access_token` snake、响应 PascalCase、路由 `Detail?id=`）见 `cube-webapi-backend` skill §14。
...
- ★ **`Lov/Meta` 响应经 http 层 camelize 后，`Meta`/`InlineEnums` 首字母已变小写** → 必须读 `data.meta` / `data.inlineEnums`
- ★ **`InlineEnums` 的"键"也被 camelize 破坏**：`Enum.X` → `enum.X`，与 `RefLovCode` 值对不上 → `refLovCode` 列翻译恒失效（显示原始 0/1/2）
...
- **★ `Lov/Meta` 响应经 http 层 camelize 后，大写键 `Meta` / `InlineEnums` 已变成 `meta` / `inlineEnums`**：`http.ts` 的响应拦截器对整个信封做 `camelize`（首字母小写），所以 `r.data.Meta` **恒为 `undefined`**。历史缺陷：`useLov.load()` 只读 `r.data.Meta` / `r.data.InlineEnums` → 值集永远拉不到、`lovListConfig` 一直空 → LIST 型字段退化成普通下拉/text，且**不报错**（静默失效，最难查）。**正确写法**：`const metaArr = Array.isArray(r.data) ? r.data : (r.data.meta ?? r.data.Meta)`；`const inline = r.data.inlineEnums ?? r.data.InlineEnums`（已修为此形态）。通用规律：**凡是被 camelize 的响应，读键一律用小写驼峰；大写键只在"数据未经 camelize"时才有意义**。

- **★ 与上一条同源、更隐蔽：`InlineEnums` 的"键"也被 camelize 破坏，导致 `refLovCode` 列字典翻译恒失效（2026-09 实测 B5）**：camelize 只把**首字母**小写，所以内联枚举字典的键 `Enum.Admin.RoleKind` → `enum.Admin.RoleKind`；而字段描述符里 `RefLovCode` 的**值**（`'Enum.Admin.RoleKind'`）是普通字符串、**保持原样**。于是 `inlineEnums['Enum.Admin.RoleKind']` 恒 `undefined` → 该列**永远显示原始值**（`1`/`2`），且不报错。**修复（useLov 已落地）**：内联枚举入表时按「本次请求的规范 lovCode」大小写不敏感复原键 —— `codes.find(c => c.toLowerCase() === rawKey.toLowerCase()) ?? rawKey.charAt(0).toUpperCase() + rawKey.slice(1)`。**验收**：任何 `refLovCode` 列必须实测出中文（LovListField 的 `textOf` 走同一字典）。注意：LovListField **不需要**改（它按 `col.refLovCode` 原值查字典，字典键复原后即命中）。

- **★ 非实体端点 `GetMenuTree` 带 `/api` 前缀；Mock/代理若只匹配无前缀路径会落到实体正则 → 404 → 侧栏恒空（2026-09 实测 A2）**：前端唯一 HTTP 层调 `getRaw('/api/Admin/Index/GetMenuTree')`（**带 `/api`**）。若 Mock 写成 `if (path === '/Admin/Index/GetMenuTree')`，则该请求不命中 → 落入实体路由正则 `^/api/([^/]+)/([^/]+)(?:/([^/]+))?$`（解析成 `area=Admin, ctrl=Index, id=GetMenuTree`）→ `handleGetPage` 404 → `MenuSidebar` 静默忽略 → **DOM 里没有任何菜单项**（`document.querySelectorAll('.t-menu__item').length === 0`）。**修复**：Mock/代理两种前缀都接受。**排障口诀**：菜单空且控制台无红错 → 先 curl 该端点看 HTTP 码（404 是路径不匹配，401 是令牌头，200 但 DOM 空是配色/渲染）。
