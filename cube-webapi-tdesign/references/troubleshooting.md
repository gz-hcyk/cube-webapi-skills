...
> 本文件收纳原 SKILL.md「常见陷阱」节（旧 §七，现 SKILL.md §六 精选版）全部条目（**保真**）。排障路径：**先看每组标题列表（症状关键词）→ 命中再读对应条目全文**。文内 `§4.x`/`§10.x` 指 SKILL.md 章节；`见 §七` 类自引用均指**本文件内其他条目**；`assets/` 指技能 assets 目录（拷贝模板源）；`references/demo/src/` 是**精简示例工程**（层次独立、**不是** scaffold 的同步目标；认证架构为上一代形态，store 内联在 `api/auth.ts`。独占资产仅注册/找回密码两页）。多数条目已注明修复落地的资产文件，表示该坑在 `assets/` 模板中已按正确做法实现，仅当自定义改动破坏约定时回查。
