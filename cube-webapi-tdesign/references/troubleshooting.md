...
> 本文件收纳原 SKILL.md「常见陷阱」节（旧 §七，现 SKILL.md §六 精选版）全部条目（**保真**）。排障路径：**先看每组标题列表（症状关键词）→ 命中再读对应条目全文**。文内 `§4.x`/`§10.x` 指 SKILL.md 章节；`见 §七` 类自引用均指**本文件内其他条目**；`assets/` 指技能 assets 目录（拷贝模板源）；`lite-demo`（lite 血统最小可运行示例工程）**已于 2026-09-13 归档移出技能**，现位于技能仓库 `.archive/cube-webapi-tdesign--demo-lite/`——它从来**不是** scaffold 的同步目标（认证架构为上一代形态，store 内联在 `api/auth.ts`；独占资产仅注册/找回密码两页）。**下文历史条目中的「（已归档）lite-demo/」即指该归档，技能内已无此目录，读到该前缀时按「历史记录」理解，勿当作可拷贝的资产路径。**多数条目已注明修复落地的资产文件，表示该坑在 `assets/` 模板中已按正确做法实现，仅当自定义改动破坏约定时回查。
...
- `required` / `nullable` / `readOnly` 语义各自独立，勿混用（高频坑）；必填为**三级判定**，`nullable` **未下发 → 默认不必填**
...
- **`required` / `nullable` / `readOnly` 语义各自独立，勿混用（高频坑）**：字段元数据属性都是**辅助前端展示**的，但含义不同：`required`=**界面是否必填**（UI 语义）、`nullable`=**数据库是否允许为空**（NOT NULL 约束）、`readOnly`=**是否只读**（控件禁用）、`visible`=是否可见（**实测后端恒为 `false`，绝不可用于判断隐藏列**）、`primaryKey`=主键（表单/列表排除）。**典型 bug**：把 `nullable===false` 直接当必填，导致 `ID`、`CreateTime`、`CreateUserID`、`Ex1/Ex2` 等 77 个字段全被标红星要求用户填写（实测 137 个表单字段中 77 个是 NOT NULL）。正确做法（**三级判定，2026-09-23 收敛**）：① `required===true` → 必填；② 否则 `nullable===false`（**明确 NOT NULL**）→ 必填；③ `nullable===true` 或 **未下发（null/undefined）** → **不必填（缺省宽松）**——即「后端没指定能否为空」时一律按**允许为空**处理。另排除主键/自增/`readOnly`/审计字段（`CreateTime`/`UpdateTime`/`CreateUserID`/`UpdateUserID`/`CreateIP`/`UpdateIP`）——它们由系统赋值。⚠️ 实测本框架对所有字段下发 `required:false`（0 个 true），**故 `required` 仅在为 `true` 时生效，不能把 `false` 当「明确不必填」**，否则连 `Name` 都不校验。⚠️ **`nullable` 是唯一非 `=== true` 的布尔位**（它要区分「未下发」第三态，判据写作 `!== false`），**勿把该写法照搬给** `primaryKey`/`readOnly`/`visible`。统一走 `fieldRender.resolveFieldBehavior()`（返回 `{required, readOnly, nullable, primaryKey}`），表单校验、`buildFormItems` **以及 `ConfigView` 的 GetFields 链路**都用它，勿各处散写 `f.nullable===false` 或 `!!f.required`。
...
- **登录相关页调用 `auth` store helper，勿在组件内动态 `import('@/api/api')`（高频坑：构建告警 + 循环依赖）**：`ForgotPasswordView`/`RegisterView`（这两页的实现样例随 lite demo 归档，见技能仓库 `.archive/cube-webapi-tdesign--demo-lite/src/pages/`）需 `resetPassword`/`registerUser` 时，应直接调用已注入的 `auth` store 方法，而非在组件内 `import('@/api/api')` 动态引入——后者触发 Vite 动态导入告警且易与 api 实例形成循环依赖（且 `@/api/api` 这一路径已按铁律 H1 删除，动态导入会直接构建失败）。`assets/core/stores/auth.ts` 已导出 `resetPassword`/`registerUser`/`sendCode`/`verifyMfa`，登录三页统一经 store 调用。
...
- **📦 前端工程实践聚合（Vite / npm / 构建 / 脚手架 / 请求层，实测于 NewLife.Cube WebApi + Vue3 + TDesign 个人博客项目）**：以下前端专属落地细节统一归口本 skill（与 `cube-webapi-backend` 第十四节后端契约互补，避免前后端知识错置）：
  ...
  - **请求层三件套完整代码不再内嵌**：`http.ts` / `camel.ts` / `auth.ts`（token 存取、`camel`/`camelize` 消费端归一、双 axios 实例 + Bearer 注入 + 401 刷新重放）真身在 `assets/`（拷贝模板源）与 `references/scaffold/src/`（生产级编排层，唯一真相源），直接拷贝该处代码，避免双份漂移。⚠️ **勿从 lite demo（已归档，`.archive/cube-webapi-tdesign--demo-lite/src/api/`）取**：demo 的 `http.ts`（9105B vs 14427B，未拆出 `./token`）/ `camel.ts`（879B vs 2306B）/ `auth.ts`（store 内联，上一代形态）均为**精简变体**（`DEMO-DIVERGENT`）；且 demo **没有** `api/token.ts`，照抄会造成「令牌能力缺失」。分页/门户两种 `data` 形状差异仍在业务层分支处理；后端契约（`access_token` snake、响应 PascalCase、路由 `Detail?id=`）见 `cube-webapi-backend` skill §14。
...
- ★★★ **根因**：技能里的业务资产有**两处镜像**（**没有第三份**；历史上另有 lite 血统 demo，已于 2026-09-13 归档移出技能），改一处必须同步另一处：
  | 层 | 件数 | 角色 | 谁消费 |
  |---|---|---|---|
  ...
  | ~~`references/demo/src/**`~~ | 28 | **已于 2026-09-13 归档移出技能** → 现位于技能仓库 `.archive/cube-webapi-tdesign--demo-lite/`；它从来**不是副本**（lite 血统另一代工程，含 7 件 ① 没有的资产） | 技能内已无该目录；`tri-diff.mjs` 默认也不再拿它当对照根（需 `--demo` 显式启用） |
...
- ★★★ **更关键的教训：跑错了闸门 —— 两个闸门判据正交，不能互相替代**：
  ...
  | `scan-assets-refs.mjs` | 技能内部 `assets/core/`(31) ↔ `scaffold/src/`(55) | 两镜像漂移、`② ⊂ ①` 被破坏、文档悬空引用 | 工程侧改动 |
...
- **硬规则（写进 SKILL.md 收尾自检）**：
  1. **改一个文件就两处一起改**（主真相源 `references/scaffold/src/` → 派生 `assets/core/`），或直接跑 `sync-assets.mjs` 一条命令派生；
     历史 lite demo 层**已归档移出技能**，不再需要同步（`tri-diff.mjs` 默认也不再看它）；
