...
//
// ⚠️ 白名单（降噪）：入口/外壳/可选/归档类文件**结构上就不会被 import**，
...
const WL_BASENAME = new Map([
  ...
  ['env.d.ts', '全局类型声明'],
  ['permissions.ts', '权限模块，经路由守卫/归档间接消费，可零静态引用'],
...
function isWhitelisted(rel) {
  ...
  for (const p of WL_PATH_PREFIX) if (rel.startsWith(p)) return '可选/归档/独立入口'
