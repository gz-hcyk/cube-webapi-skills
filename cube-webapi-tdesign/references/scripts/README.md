...
| 层 | 件数 | 角色 | 是否同步目标 |
|---|---|---|---|
| `references/scaffold/src/` | **62** | **真相源**：完整可运行工程（CLI 骨架 + 全部业务资产） | ✅ 改这里 |
| `assets/core/` | **38** | **派生镜像**：`scaffold/src` 的**严格子集**（`cp -r` 拷进工程的白名单） | ✅ 由脚本生成 |
...
- **白名单外件数**：`scaffold/src` 共 62 件 = 白名单 38 + 骨架/上游 24，数字对不上时说明有新件待归类。
